...
class Voxels(object):
    ...
    def __init__(self, data, dims, translate, scale, axis_order, height):
        self.data = data
        self.dims = dims
    ...
    def clone(self):
        ...
        height = self.height[:]
        return Voxels(data, dims, translate, self.scale, self.axis_order, height)
...
def read_header(fp):
    ...
    height = list(map(float, fp.readline().strip().split(b' ')[1:]))
    line = fp.readline()
    return dims, translate, scale, height
...
def read_as_3d_array(fp, fix_coords=True):
    ...
    """
    dims, translate, scale, height = read_header(fp)
    ...
    if fix_coords:
        ...
        axis_order = 'xzy'
    return Voxels(data, dims, translate, scale, axis_order, height)
...
def read_as_coord_array(fp, fix_coords=True):
    ...
    return Voxels(np.ascontiguousarray(data), dims, translate, scale, axis_order, height)
