...
class Model:
    ...
    def get_inner_dim (self):
        ...
        for x in range(centre[0],self.dx):
            if(self.matrix[x][centre[1]][centre[2]] == 0): self.inner_m_p[0]=x
            else : break
                
        for x in reversed(range(centre[0])):
            if(self.matrix[x][centre[1]][centre[2]] == 0): self.inner_o_p[0]=x
            else : break
        
        for y in range(centre[1],self.dy):
            if(self.matrix[centre[0]][y][centre[2]] == 0): self.inner_m_p[1]=y
            else : break
                
        for y in reversed(range(centre[1])):
            if(self.matrix[centre[0]][y][centre[2]] == 0): self.inner_o_p[1]=y
            else :break
         
        for z in range(centre[1],self.dz):
            if(self.matrix[centre[0]][centre[1]][z] == 0): self.inner_m_p[2]=z
            else : break
                
        for z in reversed(range(centre[2])):
            if(self.matrix[centre[0]][centre[1]][z] == 0): self.inner_o_p[2]=z
            else :break
